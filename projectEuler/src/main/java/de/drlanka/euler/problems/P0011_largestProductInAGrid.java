...
public class P0011_largestProductInAGrid implements EulerProblem {
  ...
  public Object solve() {
    ...
    for(int row=0;row<GRID_HEIGHT;row++)
      for(int column=0;column<GRID_WIDTH;column++) {
        ...
        max=Math.max(max, product(row, column, 1, 0));
  ...
  private int[][] readFromGrid(int rows, int columns) {
    String[] split = grid.split("\\s+");
    int[][] ints=new int[rows][columns];
    int i=0;
    for(int row=0;row<rows;row++) {
      for(int column=0;column<columns;column++)
        ints[row][column]=Integer.valueOf(split[i++]).intValue();
    }
    return ints;
  }
  ...
  int readHorizontal() {
    int max=0;
    for(int row=0;row<GRID_HEIGHT;row++) {      
      for(int col=0;col<GRID_WIDTH-productLength;col++) {
        int product=1;
  ...
  private long product(int row, int column, int rowStep, int columnStep) {
    ...
    int columnEnd=column+columnStep*productLength;
